...
object DSLRegistry {
  ...
  fun inferCalleeClass() : String {
    val clazz = Thread
            ...
            .getStackTrace()
            .asSequence()
            .drop(1) /* 0 => #getStacktrace + several Kotlin-generated wrappers are on stack */
            .firstOrNull { !it.getClassName().startsWith("org.jonnyzzz.teamcity.dsl.api.") }
            ?: throw Error("Failed to resolve callee package. Incorrect/empty package was used?")
    return clazz.getClassName()
  }

  fun addCompletedProject(clazz : String = inferCalleeClass(), p : TCProject) : Unit {
  ...
  fun addOnCompletedUUIDCallback(clazz : String = inferCalleeClass(), callback : (TCUUID) -> Unit) {
